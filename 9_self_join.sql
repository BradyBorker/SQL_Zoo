...
-- Craiglockhart (53). Run the query and notice the two services that link these 
...
FROM 
  ...
  WHERE stopa.name='Craiglockhart') S
JOIN
  (SELECT a.num, a.company, b.stop
  FROM route a JOIN route b
  ON a.company=b.company AND a.num=b.num
  JOIN stops stopa ON stopa.id=a.stop
  WHERE stopa.name='Lochend') E
ON S.stop=E.stop
JOIN stops ON stops.id=S.stop
ORDER BY S.num, S.company, stops.name, E.num